...
def generate_branding_insight(emotion_summary, keyword):
    prompt = f"""
...
def generate_emotion_pdf(summary, keyword):
    ...
    pdf.set_font("Arial", "", 12)
    pdf.multi_cell(0, 10, (
        "Use the emotional trends above to adjust your brand messaging.
"
        "- Reuse customer phrases to show empathy.
"
        "- Turn pain points into persuasive posts.
...
def run_emotion_engine(keyword, user_email):
    ...
    branding_summary = generate_branding_insight(emotion_summary, keyword)
    ...
    print(f"â Emotion Engine completed for '{keyword}'. PDF saved as: {pdf_file}")
